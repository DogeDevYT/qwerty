...
use crate::ast::{
    ...
    qpu::{
        Adjoint, Basis, BasisTranslation, BitLiteral, Compose, EmbedClassical, EmbedKind, Expr,
        Measure, NonUniformSuperpos, Pipe, Predicated, QLit, QLitExpr, QubitRef, Tensor, Tilt,
        UnitLiteral, Variable, Vector, expr,
    },
};
...
impl ReplState {
    ...
    pub fn run(&mut self, stmt: &Stmt<Expr>) -> Result<Expr, NotImplementedError> {
        match stmt {
            ...
            unknown => Err(NotImplementedError(format!(
                "Unknown type of statement {}. Sorry",
                unknown
            ))),
        }
    }
    ...
    pub fn eval_step(&self, state: &mut ReplState) -> Result<Expr, NotImplementedError> {
        ...
        if vals.len() == 1 {
            let val = vals
                .into_iter()
                .next()
                .expect("vals was length 1 but is empty, how?");
            Ok(val)
...
impl Expr {
    ...
    pub fn eval_to_value(&self, state: &mut ReplState) -> Result<Expr, NotImplementedError> {
        ...
        if val.is_value() {
            ...
            Err(NotImplementedError(format!(
